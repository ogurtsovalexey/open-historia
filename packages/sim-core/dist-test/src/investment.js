 ...
 */
export function calculateInvestmentPreview(region, polityStocks, constants, spendMicros, targetRegionId, actorPolityId) {
    ...
    if (!isValidTarget || !canAfford) {
        return {
            ...
            estimatedNextMonthOutputDelta: 0,
            affectedNationalCommodityTotal: 0,
            canAfford,
            isValidTarget
        };
    }
    ...
    const newInfrastructureBp = Math.min(region.infrastructureBp + infrastructureGain, constants.maxInfrastructureBp);
    const infrastructureChange = newInfrastructureBp - region.infrastructureBp;
    // Calculate estimated next month output delta
    const estimatedOutputDelta = calculateRegionOutputDelta(region, newInfrastructureBp, constants);
 ...
 */
function calculateRegionOutputDelta(region, newInfrastructureBp, constants) {
    ...
    return (newOutput - currentOutput);
}
/**
 * Calculate region gross output as defined in spec formulas
 */
export function calculateRegionGrossOutput(region) {
    ...
    return Math.max(0, grossOutput);
}
/**
 * Validate investment command parameters
 */
export function validateInvestmentCommand(region, polityStocks, spendMicros, targetRegionId, actorPolityId) {
    const errors = [];

...
export const PLAYER_INPUT_SYSTEM_PROMPT = [
  ...
  "Every requestedAction must select operation process.propose, military.mobilize, diplomacy.propose, or territory.offer. Use military.mobilize only for a direct request to raise or mobilize a reserve; it has no numeric fields because the engine chooses the bounded personnel and controlled origin. For diplomacy select only published polity, region, and relationship type IDs; never supply access percentages, control profiles, combat, peace, GM authority, or any numeric effect.",

...
function decisionToolSchema(brief) {
  ...
  process.directionId = { type: 'string', enum: unique(brief.processOptions.flatMap((option) => option.allowedDirections.map((direction) => direction.directionId))) };
  process.pace = { type: 'string', enum: unique(brief.processOptions.flatMap((option) => option.allowedPaces)) };
  process.effectFamilies.items = { type: 'string', enum: unique(brief.processOptions.flatMap((option) => option.compatibleEffectFamilies)) };
  process.targetEntityRefs.items = { type: 'string', enum: unique(brief.processOptions.flatMap((option) => option.allowedTargetEntityRefs)) };
  ...
  initiative.type = { type: 'string', enum: brief.initiativeEnvelope.allowedConceptTypes };
  initiative.directionId = { type: 'string', enum: brief.initiativeEnvelope.allowedDirectionIds };
  initiative.domainIds.items = { type: 'string', enum: brief.initiativeEnvelope.allowedDomains };
  initiative.sponsorEntityRefs.items = { type: 'string', enum: brief.initiativeEnvelope.allowedSponsorEntityRefs };
  initiative.affectedEntityRefs.items = { type: 'string', enum: brief.initiativeEnvelope.allowedTargetEntityRefs };
  initiative.pace = { type: 'string', enum: ['slow', 'steady', 'urgent'] };
  initiative.effectFamilies.items = { type: 'string', enum: brief.initiativeEnvelope.allowedEffectFamilies };

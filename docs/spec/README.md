# Specification Index

> **SUPERSEDED (2026-08-30):** this corpus is archived reference material.
> The binding, compressed source of truth is [`docs/canon/`](../canon/README.md).
> If this document and canon disagree, canon wins.

## Authority

1. `../principles.md` — binding constraints.
2. `consensus-spec.md` — accepted Phase 1 scope and decisions.
3. `acceptance-criteria.md` — implementation gates.
4. `ai-call-registry.md` — accepted observable AI task, context, budget and
   cost-ledger contract for AC-1.
5. `atomic-world-revision.md` — accepted atomic publish, recovery, rollback and
   compatibility contract for AC-2.
6. `phase1-test-plan.md` — evidence-grounded executable QA matrix mapping AC-1 through AC-9.
7. `first-economy-mvp.md` and `regional-resource-economy.md` — accepted
   owner-directed pre-historical playtest contracts.
8. `recovered/` — verbatim agent evidence, not implementation authority.

The global roadmap and research corpus is in `../product/`.

## Status

- Discovery and design: complete.
- Developer/AI consensus review: complete.
- PO review: complete after consolidation amendments.
- Implementation: not started.
- Remote publication: blocked because the configured fork is public.

## Documents

- [Discovery summary](discovery-summary.md)
- [Accepted consensus](consensus-spec.md)
- [Acceptance criteria](acceptance-criteria.md)
- [AI call registry contract](ai-call-registry.md)
- [Atomic world revision contract](atomic-world-revision.md)
- [First economy MVP](first-economy-mvp.md)
- [Regional resource economy extension](regional-resource-economy.md)
- [QA plan](phase1-test-plan.md) — evidence-grounded executable matrix mapping AC-1 through AC-9 bidirectionally
- [Recovery provenance](provenance.md)

...
const wireActionSchema = z.object({
  ...
  targetRegionId: z.string().nullable().optional(), scale: z.enum(['small', 'medium', 'large']).optional(),
  ...
  factionId: z.string().optional(), templateId: z.string().optional(), targetPolityId: z.string().optional(), commanderId: z.string().nullable().optional(),
...
const wireDecisionSchema = z.object({
  ...
  contingency: z.string(), rationale: z.string(), intendedOutcome: z.string().optional(),
  hold: z.object({ reason: z.enum(['no-legal-action', 'waiting-response', 'insufficient-resources', 'plan-sequencing', 'risk-too-high']), detail: z.string(),
    revisit: z.object({ afterMonths: z.number().int().min(1).max(12), triggers: z.array(z.enum(['resource-deficit', 'diplomatic-response', 'war', 'occupation', 'peace', 'crisis', 'government-change', 'default'])).min(1).max(8) }).strict() }).strict().nullable(),
}).strict();

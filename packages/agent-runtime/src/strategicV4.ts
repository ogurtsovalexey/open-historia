...
export interface StrategicBriefV4 {
  ...
  publicData: { economy: StrategicBriefV3['economy']; commitments: string[] };
...
export function buildStrategicBriefV4(state: EconWorldState, polityId: string, options: StrategicBriefV4Options): StrategicBriefV4 {
  ...
  const choices = allActions.filter((action) => relevant.has(action.tool)).map((action) => {
    ...
    for (const affordance of v3.affordances) {
      ...
      const visit = (candidate: { action: StrategicActionV2; preview: StrategicPreviewV3 }) => {
        if (JSON.stringify(candidate.action) === serialized) preview = candidate.preview;
      };
      if (affordance.tool === 'conserve') visit(affordance.choice);
      else if (affordance.tool === 'invest') affordance.regions.flatMap((entry) => entry.scales).forEach(visit);
      else if (affordance.tool === 'reallocate-production') affordance.regions.flatMap((entry) => entry.priorities.flatMap((priority) => priority.scales)).forEach(visit);
      else if (affordance.tool === 'negotiate-trade' || affordance.tool === 'external-import') affordance.partners.flatMap((entry) => entry.resources.flatMap((resource) => resource.choices)).forEach(visit);
      else if (affordance.tool === 'propose-agreement') affordance.partners.flatMap((entry) => entry.choices).forEach(visit);
      else if (affordance.tool === 'apply-diplomatic-pressure') affordance.partners.flatMap((entry) => entry.choices).forEach(visit);
      else if (affordance.tool === 'respond-proposal') affordance.proposals.flatMap((entry) => entry.choices).forEach(visit);
      else if (affordance.tool === 'change-policy') affordance.choices.forEach(visit);
      else if (affordance.tool === 'respond-faction') affordance.factions.flatMap((entry) => entry.choices).forEach(visit);
      else if (affordance.tool === 'start-project') affordance.projects.flatMap((entry) => entry.choices).forEach(visit);
      else if (affordance.tool === 'mobilize') affordance.regions.flatMap((entry) => entry.choices).forEach(visit);
      else if (affordance.tool === 'declare-war') affordance.defenders.flatMap((entry) => entry.reasons.map((reason) => reason.choice)).forEach(visit);
      else if (affordance.tool === 'issue-order') affordance.formations.flatMap((entry) => entry.choices).forEach(visit);
...
export function materializeStrategicDecisionV4(state: EconWorldState, raw: unknown, brief: StrategicBriefV4): StrategicResolutionV4 {
  ...
  const commands = actions.flatMap((action) => mapStrategicActionToCommandsUncheckedV3(state, brief.actor.id, action));
  if (commands.length !== actions.length) return { status: 'terminal', commands: [], decision: null, pendingTriggerIds: pending, reason: 'deterministic expansion failed' };

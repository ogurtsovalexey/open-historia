...
// The adapter resolves DATA_DIR from OH_DATA_DIR at module evaluation, so we
// import it lazily after the env variable is set in the before() hook.
let readGameStateBundle;
let commitWorldRevision;
...
describe("World Revision Filesystem Adapter", () => {
  before(async () => {
    ...
    // Override the writable data root before the adapter module is evaluated.
    process.env.OH_DATA_DIR = TEST_DATA_DIR;
    const adapter = await import("./worldRevisionFilesystem.js");
    readGameStateBundle = adapter.readGameStateBundle;
    commitWorldRevision = adapter.commitWorldRevision;
  ...
  describe("atomic-world-revision contract cases", () => {
    test("case 5: explicit gameId isolates concurrent games", async () => {
      const projections = createValidProjections();
      const gameA = "test-game-a";
      const gameB = "test-game-b";

      const resultA = await commitWorldRevision({
        gameId: gameA,
        expectedRevision: null,
        reason: "pregame",
        projections,
      });
      const resultB = await commitWorldRevision({
        gameId: gameB,
        expectedRevision: null,
        reason: "pregame",
        projections: { ...projections, game: { ...projections.game, turn: 7 } },
      });

      assert.equal(resultA.status, "committed");
      assert.equal(resultB.status, "committed");

      const bundleA = await readGameStateBundle(gameA);
      const bundleB = await readGameStateBundle(gameB);

      assert.notEqual(bundleA.manifest.revision, bundleB.manifest.revision);
      assert.equal(bundleA.projections.game.turn, 1);
      assert.equal(bundleB.projections.game.turn, 7);
    });

    test("case 6: legacy baseline survives first revisioned write failure", async () => {
      const legacyGameId = "test-legacy-game";
      const storageDir = path.join(TEST_DATA_DIR, "games", legacyGameId, "storage");
      fs.mkdirSync(storageDir, { recursive: true });

      const legacy = createValidProjections();
      for (const [key, value] of Object.entries(legacy)) {
        fs.writeFileSync(path.join(storageDir, `${key}.json`), JSON.stringify(value), "utf-8");
      }

      // Import the manifest-less save as a synthesized baseline.
      const baseline = await readGameStateBundle(legacyGameId);
      assert(baseline.manifest.revision.startsWith("legacy-"));

      // First revisioned write fails validation.
      await assert.rejects(
        () => commitWorldRevision({
          gameId: legacyGameId,
          expectedRevision: baseline.manifest.revision,
          reason: "compat-write",
          projections: { ...legacy, actions: undefined },
        }),
        (error) => error instanceof WorldRevisionError && error.code === "UNDEFINED_PROJECTION"
      );

      // The legacy baseline remains readable and unchanged.
      const after = await readGameStateBundle(legacyGameId);
      assert.equal(after.manifest.revision, baseline.manifest.revision);
      assert(after.manifest.revision.startsWith("legacy-"));

      // A racing stale writer conflicts instead of overwriting.
      const conflicted = await commitWorldRevision({
        gameId: legacyGameId,
      ...
      assert.equal(conflicted.currentRevision, baseline.manifest.revision);
    });

    test("case 8: rollback publishes a new revision and failed rollback changes nothing", async () => {
      const projections = createValidProjections();
      ...
      // Rollback never moves the pointer backward; it publishes a new revision.
      const rollback = await commitWorldRevision({
        gameId: TEST_GAME_ID,
        expectedRevision: rev2,
        reason: "rollback",
        rollbackOf: rev1,
      ...
      assert.equal(rollback.bundle.manifest.reason, "rollback");
      assert.equal(rollback.bundle.manifest.rollbackOf, rev1);
      assert.equal(rollback.bundle.manifest.parentRevision, rev2);

      const rollbackRevision = rollback.bundle.manifest.revision;
      assert.notEqual(rollbackRevision, rev1);
      assert.notEqual(rollbackRevision, rev2);

      // A failed rollback leaves the current revision and history unchanged.
      const before = await readGameStateBundle(TEST_GAME_ID);
      await assert.rejects(
        () => commitWorldRevision({
          gameId: TEST_GAME_ID,
          ...
          reason: "rollback",
          rollbackOf: rev1,
          projections: { ...projections, actions: undefined },
        }),
        (error) => error instanceof WorldRevisionError
      );

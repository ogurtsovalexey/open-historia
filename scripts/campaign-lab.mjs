...
const geminiDecision = async ({ id, manifest, batch, correction = null }) => {
  ...
  const system = `Play each requested polity independently. Return strategic decision JSON. Select zero to three compatible material tools from: ${CAMPAIGN_DECISION_TOOLS.join(", ")}. Each wire action uses only tool plus target (region/proposal/faction/template/formation/war), counterpart (polity), subject (resource/budget/commander/target region), choice (priority/runway/agreement/demand/response/stance/reason/posture/approach), and intensity (scale or budget attitude), as applicable. Choose qualitative goals, partners, risk, sequence and contingency; never emit engine commands, technical ids, numeric effects, values outside the brief, or geometry. If no material action is justified, use conserve plus a typed hold reason and revisit triggers. Omit hold for material actions. Unsupported consequences belong only in intendedOutcome and must not be narrated as completed.`;

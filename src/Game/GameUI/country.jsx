...
export const Country = ({ playerPolity, facts, locale }) => (
  <section className="oh-intent-section" aria-labelledby="intent-country-title" data-testid="intent-surface-country">
    ...
    {facts.length > 0 ? facts.map((fact) => <GroundedValue fact={fact} key={fact.factId} locale={locale} />) : <div className="oh-intent-empty">{intentText(locale, "No grounded country facts are available.")}</div>}
  </section>
);

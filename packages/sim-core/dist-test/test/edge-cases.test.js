...
import { calculateInvestmentPreview, validateInvestmentCommand } from '../src/investment.js';
...
describe('Edge Cases and Mutation Resistance', () => {
    ...
    describe('Investment Edge Cases', () => {
        it('should handle zero spend investment', () => {
            ...
            const preview = calculateInvestmentPreview(region, polityA, constants, 0, region.regionId, polityA.polityId);
        ...
        it('should handle maximum infrastructure investment', () => {
            ...
            const nearlyMaxRegion = {
                ...region,
                infrastructureBp: 9900 // 99%
            };
            const largeSpend = 1000000000; // Large spend
            const preview = calculateInvestmentPreview(nearlyMaxRegion, polityA, constants, largeSpend, region.regionId, polityA.polityId);
            // Infrastructure change should be clamped at max
            assert(preview.infrastructureChange <= 100); // Can't exceed 10000 bp total
        });
        it('should handle negative population edge case', () => {
            // This test ensures we don't have negative population
            const regionWithLowPopulation = {
                ...regions[0],
                population: 10,
    ...
    describe('Monthly Resolution Edge Cases', () => {
        ...
        it('should handle polity with negative treasury (should not happen but defensive)', () => {
            ...
            assert(result.nextPolityStocks.length === 1);
        });
        it('should handle accepted investment with insufficient treasury', () => {
            ...
            // Should skip investment but not crash
            const result = resolveMonth(regions, modifiedPolityStocks, constants);
            assert.strictEqual(result.nextPolityStocks[0].treasuryMicros, 0);
        });
    });
    describe('Region Transfer Edge Cases', () => {
        it('should handle transfer to non-existent polity', () => {
            assert.throws(() => {
                processRegionTransfer(regions, polityStocks, regions[0].regionId, 'polity:test-a', 'polity:non-existent');
            }, /To polity .* not found/);
        });
        it('should handle transfer from non-existent polity', () => {
            assert.throws(() => {
                processRegionTransfer(regions, polityStocks, regions[0].regionId, 'polity:non-existent', 'polity:test-b');
            }, /From polity .* not found/);
        });
        it('should handle circular transfer (A -> B -> A)', () => {
            // First transfer A -> B
            const result1 = processRegionTransfer(regions, polityStocks, regions[0].regionId, 'polity:test-a', 'polity:test-b');
            // Then transfer back B -> A
            const result2 = processRegionTransfer(result1.updatedRegions, result1.updatedPolityStocks, regions[0].regionId, 'polity:test-b', 'polity:test-a');
        ...
        it('should preserve remainders during multiple transfers', () => {
            const regionWithRemainders = {
                ...regions[0],
                birthRemainder: 50000n,
                deathRemainder: 30000n
            };
            const modifiedRegions = [regionWithRemainders];
            const result = processRegionTransfer(modifiedRegions, polityStocks, regionWithRemainders.regionId, 'polity:test-a', 'polity:test-b');
            const transferredRegion = result.updatedRegions[0];
            assert.strictEqual(transferredRegion.birthRemainder, 50000n);
            assert.strictEqual(transferredRegion.deathRemainder, 30000n);
        });
    });
    describe('Deterministic Behavior', () => {
        it('should produce identical results for identical inputs', () => {
            ...
            const json1 = JSON.stringify(result1, (key, value) => typeof value === 'bigint' ? value.toString() : value);
            const json2 = JSON.stringify(result2, (key, value) => typeof value === 'bigint' ? value.toString() : value);
            assert.strictEqual(json1, json2);
        });
        it('should produce identical transfer results for identical inputs', () => {
            const result1 = processRegionTransfer(regions, polityStocks, regions[0].regionId, 'polity:test-a', 'polity:test-b');
            const result2 = processRegionTransfer(regions, polityStocks, regions[0].regionId, 'polity:test-a', 'polity:test-b');
            const json1 = JSON.stringify(result1, (key, value) => typeof value === 'bigint' ? value.toString() : value);
            const json2 = JSON.stringify(result2, (key, value) => typeof value === 'bigint' ? value.toString() : value);
            assert.strictEqual(json1, json2);
        });
        it('should handle large numbers without overflow', () => {
            ...
            assert(workforce > 0);
            assert(labourOutput > 0);
            assert(!isNaN(labourOutput));
            assert(isFinite(labourOutput));
        });
    });
    describe('Validation Edge Cases', () => {
        it('should validate investment with mismatched region ID', () => {
            const region = regions[0];
            const polityA = polityStocks[0];
            const validation = validateInvestmentCommand(region, polityA, 100000000, 'region:test:wrong-id', // Wrong ID
            polityA.polityId);
            assert.strictEqual(validation.valid, false);
            assert(validation.errors.some(e => e.includes('mismatch')));
        });
        it('should validate investment with zero spend', () => {
            const region = regions[0];
            const polityA = polityStocks[0];
            const validation = validateInvestmentCommand(region, polityA, 0, region.regionId, polityA.polityId);
            assert.strictEqual(validation.valid, false);
            assert(validation.errors.some(e => e.includes('positive')));
        });
